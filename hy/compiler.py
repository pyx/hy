...
from hy.models import (HyObject, HyExpression, HyKeyword, HyInteger, HyComplex,
                       ...
                       HyDict, HySequence, wrap_value, spoof_positions)
from hy.model_patterns import (EXPR, SYM, STR, sym, sb, whole, notform,
                               dolike, form, times)
...
_bad_roots = tuple(ast_str(x) for x in (
    "unquote", "unquote-splicing", "unpack-mapping", "except"))
...
class HyASTCompiler(object):
    ...
    @special([(not PY3, "exec*")], [EXPR, maybe(EXPR), maybe(EXPR)])
    ...
    @special("do", [many(EXPR)])
    def compile_do(self, expr, root, body):
        return self._compile_branch(body)

    @special("raise", [maybe(EXPR), maybe(sym(":from") + EXPR)])
    ...
    @special("try",
       [many(notform("except", "else", "finally")),
        many(form(sym("except"),
            sb() | sb(EXPR) | sb(SYM, EXPR),
            many(EXPR))),
        maybe(dolike("else")),
    ...
    @special("if*", [EXPR, EXPR, maybe(EXPR)])
    ...
    @special("cut", [EXPR, maybe(EXPR), maybe(EXPR), maybe(EXPR)])
    ...
    @special("with-decorator", [oneplus(EXPR)])
    ...
    @special(["list-comp", "set-comp", "genexpr"], [EXPR, EXPR, maybe(EXPR)])
    ...
    @special("dict-comp", [EXPR, EXPR, EXPR, maybe(EXPR)])
    ...
    @special(["import", "require"], [many(
        SYM |
        sb(SYM, sym(":as"), SYM) |
        sb(SYM, sb(many(SYM + maybe(sym(":as") + SYM)))))])
    ...
    @special(["=", "is", "<", "<=", ">", ">="], [oneplus(EXPR)])
    @special(["!=", "is-not"], [times(2, Inf, EXPR)])
    @special(["in", "not-in"], [times(2, 2, EXPR)])
    ...
    @special(["+", "*", "|"], [many(EXPR)])
    @special(["-", "/", "&", (PY35, "@")], [oneplus(EXPR)])
    @special(["**", "//", "<<", ">>"], [times(2, Inf, EXPR)])
    @special(["%", "^"], [times(2, 2, EXPR)])
    ...
    @special(["for*", (PY35, "for/a*")],
             [sb(EXPR, EXPR), many(notform("else")), maybe(dolike("else"))])
    ...
    @special(["while"], [EXPR, many(notform("else")), maybe(dolike("else"))])
    ...
    @special(["fn", "fn*", (PY35, "fn/a")], [
        ...
        sb(
            many(NASYM),
            maybe(sym("&optional") + many(NASYM | sb(SYM, EXPR))),
            maybe(sym("&rest") + NASYM),
            maybe(sym("&kwonly") + many(NASYM | sb(SYM, EXPR))),
    ...
    @special("defclass", [
        SYM,
        maybe(sb(many(EXPR)) + maybe(STR) +
              maybe(sb(many(SYM + EXPR))) + many(EXPR))])
